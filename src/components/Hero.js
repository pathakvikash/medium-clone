...
function Hero() {
  return (
    <div>
      <header className='bg-yellow-400 py-8 flex items-center p-9 h-[550px]'>
        <div className='flex flex-col pt-9 '>
          <b>
            <p className='text-[85px] font-[400] pb-6 font-georgia font-gt-super font-cambria font-times-new-roman font-times font-serif'>
              Stay curious.
            </p>
          </b>
...
export function TrendingSection() {
  ...
  const cardLayout = (article, index) => (
    <div
      key={article.id}
    ...
    >
      ...
      <div className='flex flex-col'>
        ...
        <p className='text-gray-500 mb-2 text-sm'>{`${article.date} · ${article.readTime} min read`}</p>
      </div>
    </div>
  );
